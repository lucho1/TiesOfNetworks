...
void ModuleSound::RenderAudio(sound_output_buffer *Buffer, uint32 advancedSamples)
{
	...
	for (uint32 audioSourceIndex = 0; audioSourceIndex < ArrayCount(m_AudioSources); ++audioSourceIndex)
	{
		audio_source &audioSource = m_AudioSources[audioSourceIndex];
		...
		if (audioSourceSampleIndex < audioClipSampleCount || audioSource.flags & AUDIO_SOURCE_LOOPS_BIT)
		{
...
void ModuleSound::OutputAudio(sound_output *SoundOutput, sound_output_buffer *SourceBuffer, DWORD ByteToLock, DWORD BytesToWrite)
{
	...
	if (SoundBuffer->Lock(ByteToLock, BytesToWrite, &Region1, &Region1Size,	&Region2, &Region2Size,	0) == DS_OK)
	{
...
bool ModuleSound::PostUpdate()
{
	...
	if (SoundBuffer->GetCurrentPosition(&PlayCursor, &WriteCursor) == DS_OK)
	{
		...
		DWORD AdvancedBytes = (ByteToLock > SoundOutput.LastByteToLock) ? ByteToLock - SoundOutput.LastByteToLock :
		...
		SoundOutputBuffer.Samples = m_Samples;
...
{
	for (uint32 i = 0; i < ArrayCount(m_AudioClips); ++i)
	{
		free(m_AudioClips[i].samples);
		m_AudioClips[i] = {};
	}
...
AudioClip * ModuleSound::LoadAudioClip(const char * filename)
{
	...
	for (uint32 i = 0; i < ArrayCount(m_AudioClips); ++i)
	{
		if (m_AudioClips[i].samples == nullptr)
		{
			audioClip = &m_AudioClips[i];
			break;
		}
	}
...
void ModuleSound::PlayAudioClip(AudioClip * audioClip)
{
	...
	for (uint32 i = 0; i < ArrayCount(m_AudioSources); ++i)
	{
		if (m_AudioSources[i].clip == audioClip)
		{
			m_AudioSources[i].lastWriteSampleIndex = 0;
			m_AudioSources[i].flags = AUDIO_SOURCE_START_BIT;
			return;
		}
	}
	...
	for (uint32 i = 0; i < ArrayCount(m_AudioSources); ++i)
	{
		if (m_AudioSources[i].clip == nullptr)
		{
