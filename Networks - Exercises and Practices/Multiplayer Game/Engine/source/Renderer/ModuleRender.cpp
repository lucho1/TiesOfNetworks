...
bool ModuleRender::Init()
{
	...
	LoadShaderFile("vertex_shader.hlsl", m_ShaderSource, &shaderSourceLength);
...
bool ModuleRender::Update()
{
	...
	uint32 prevSpriteCount = m_SpriteCount;

	for (uint32 i = 0; i < ArrayCount(m_Sprites); ++i)
	{
		GameObject *gameObject = m_Sprites[i].gameObject;
		if (gameObject != nullptr)
		{
			if (gameObject->state == GameObject::DESTROYING)
				RemoveSprite(gameObject);

			if (++spritesVisited >= prevSpriteCount)
				break;
		}
	}

	uint32 animationsVisited = 0;
	uint32 prevAnimationCount = m_AnimationCount;

	for (uint32 i = 0; i < ArrayCount(m_Animations); ++i)
	{
		GameObject *gameObject = m_Animations[i].gameObject;
		if (gameObject != nullptr)
		{
			if (gameObject->state == GameObject::DESTROYING)
				RemoveAnimation(gameObject);

			if (++animationsVisited >= prevAnimationCount)
				break;
		}
	}
...
{
	ASSERT(m_Sprites[parent->id].gameObject == nullptr);

	Sprite *sprite = &m_Sprites[parent->id];
...
void ModuleRender::RemoveSprite(GameObject * parent)
{
	...
	m_Sprites[parent->id].gameObject->sprite = nullptr;
	m_Sprites[parent->id] = {};
	m_SpriteCount--;
}
...
{
	ASSERT(m_Animations[parent->id].gameObject == nullptr);

	Animation *animation = &m_Animations[parent->id];
...
void ModuleRender::RemoveAnimation(GameObject * parent)
{
	...
	m_Animations[parent->id].gameObject->sprite = nullptr;
	m_Animations[parent->id] = {};
	m_AnimationCount--;
}
...
AnimationClip * ModuleRender::AddAnimationClip()
{
	...
	{
		AnimationClip &clip = m_AnimationClips[i];
...
AnimationClip * ModuleRender::GetAnimationClip(uint16 animationClipId)
{
	...
	return &m_AnimationClips[animationClipId];
}
...
void ModuleRender::RenderScene()
{
	...
	// Render game objects
	int numObjects = 0;
	SelectAndSortObjects(App->modGameObject->GameObjects, m_OrderedGameObjects, &numObjects);
	...
	for (int i = 0; i < numObjects; ++i)
	{
		...
		// Setup matrices into our constant buffer
		{
			...
			vec2 size = vec2{gameObject->size.x == 0.0f ? textureSize.x : gameObject->size.x, gameObject->size.y == 0.0f ? textureSize.y : gameObject->size.y };
		...
		Texture *texture = gameObject->sprite->texture ? gameObject->sprite->texture : m_WhitePixel;
	...
	if (mustRenderColliders)
	{
		...
		// Pass texture to shader
		Texture* texture = m_WhitePixel;
		ID3D11ShaderResourceView* texture_srv = (ID3D11ShaderResourceView*)texture->shaderResource;
		ctx->PSSetShaderResources(0, 1, &texture_srv);

		for (uint32 i = 0; i < MAX_COLLIDERS; ++i)
		{
			Collider *collider = &App->modCollision->m_Colliders[i];
			GameObject *gameObject = collider->gameObject;

			if (collider->type == ColliderType::NONE || gameObject == nullptr)
				continue;
